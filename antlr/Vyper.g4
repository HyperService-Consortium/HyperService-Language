...
import_from
  :('from' (('.' | '...')* dotted_name | ('.' | '...')+)
                 'import' ('*' | '(' importDeclarations ')' | importDeclarations));
...
extracting_Interfaces
  : 'vyper' '-f' ('interface' | 'external_interface') identifier ('/' identifier)* '.vy'
  ;

contract_Interfaces
  : 'contract' identifier ':' contract_def*
  ;
...
parameter
  : parName = identifier ':' parType = type (',')?
  ;
...
numberLiteral
  : (IntegerNumber | DecimalNumber | HexNumber) NumberUnit? ;
